...
Retrieve Customer by ID, List all Customers
Invoice
Create (Draft), Update, Mark as Sent, Record Payment
Retrieve Invoice by ID, List Invoices by Status/Customer
Payment
Record Payment (Applies to Invoice)
Retrieve Payment by ID, List Payments for an Invoice
...
Demo: A video or live presentation demonstrating the core functional flow (Customer creation, Invoice creation with line items, Payment application).
Brief Technical Writeup (1-2 pages): Documenting the chosen architecture (DDD boundaries, CQRS implementation, VSA structure), design decisions, and database schema.
AI Tool Documentation: Detailed documentation of the specific AI tools used, including example prompts and a justification for how they accelerated development while maintaining architectural quality.
